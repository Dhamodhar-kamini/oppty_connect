...
function PollMessage({ message, onVote, myUserId }) {
  ...
  return (
    <div className={`pollMessageContainer ${isMine ? "pollMine" : "pollTheirs"}`}>
      ...
      <div className="pollOptionsList">
        {pollData.map((option, index) => {
          ...
          return (
            <button
              key={option.id || index}
              className={`pollOptionBtn ${isSelected ? "pollOptionSelected" : ""} ${
                hasVoted ? "pollOptionVoted" : ""
              }`}
            ...
            >
              <div className="pollOptionContent">
                <div className="pollOptionLeft">
                  <div
                    className={`pollCheckCircle ${
                      isSelected ? "pollCheckCircleSelected" : ""
                    }`}
                  >
                ...
                {hasVoted && (
                  <span className="pollVoteCount">{voteCount}</span>
                )}
              </div>
              {hasVoted && (
                <div className="pollProgressBar">
                  <div
                    className={`pollProgressFill ${
                      isSelected ? "pollProgressFillSelected" : ""
                    }`}
      ...
      <div className="pollFooter">
        <span className="pollTotalVotes">
          {totalVotes} vote{totalVotes !== 1 ? "s" : ""}
        </span>
        {message.allowMultiple && (
          <span className="pollMultipleHint">• Select multiple</span>
        )}
      </div>
...
export default function ChatPage() {
  ...
  const isGroup = useMemo(
    () =>
      String(chatId).startsWith("group-") ||
  ...
  const handleCreatePoll = async () => {
    ...
    if (!pollQuestion.trim()) {
      showToast("Please add a question", "error");
      return;
    }
    ...
    try {
      ...
      setLocalMeets((prev) => [...prev, pollMessage]);
  ...
  const handlePollVote = useCallback(
    async (messageId, optionId, optionIndex) => {
  ...
  // Listen for WebSocket poll_update events
  useEffect(() => {
    const handlePollUpdate = (event) => {
      if (event.detail) {
        ...
        setLocalPollUpdates((prev) => ({
          ...
          [data.message_id]: {
            ...
            myVotes: data.myVotes || prev[data.message_id]?.myVotes || [],
    ...
    window.addEventListener("poll_update", handlePollUpdate);
    return () => window.removeEventListener("poll_update", handlePollUpdate);
  }, []);
  ...
  const combinedMessages = useMemo(() => {
    ...
    const processMessage = (m) => {
      ...
      // Normalize type/messageType consistency
      ...
      // Normalize isMine if missing
      if (msg.isMine === null || msg.isMine === undefined) {
        if (msg.sender === "me") {
          msg.isMine = true;
      ...
      if (localDeletesForAll.has(m.id)) {
        ...
        msg.fileUrl = null;
        msg.pollOptions = null;
        msg.pollQuestion = null;
      ...
      if (
        (msg.type === "poll" || msg.messageType === "poll") &&
        localPollUpdates[m.id]
      ) {
        const update = localPollUpdates[m.id];
        msg.pollOptions = update.pollOptions;
        msg.myVotes = update.myVotes;
        msg.totalVotes = update.totalVotes;
      }

      if (localThreads[m.id]) {
        const existingIds = new Set((msg.thread || []).map((t) => t.id));
        const newThreads = localThreads[m.id].filter(
  ...
  const combinedMessagesRef = useRef(combinedMessages);
  useEffect(() => {
  ...
  const pinnedMessages = useMemo(
    () => combinedMessages.filter((m) => m.isPinned && !m.deletedForAll),
    [combinedMessages]
  );
  const mediaMessages = useMemo(
    () =>
      combinedMessages.filter(
        (m) =>
          ["image", "video", "document"].includes(m.type) || m.fileUrl
      ),
    [combinedMessages]
  );
  const activeThreadMsg = useMemo(
    () =>
      activeThreadMsgId
        ? combinedMessages.find((m) => m.id === activeThreadMsgId)
        : null,
    [combinedMessages, activeThreadMsgId]
  );
  ...
  const commonGroups = useMemo(() => {
    ...
    return chats.filter(
      (c) =>
        c.kind === "group" &&
        c.members?.some((m) => String(m.id) === targetId)
    );
  }, [chats, targetId, isGroup]);

  const isGroupAdmin =
    isGroup &&
    (chatInfo?.members?.find(
  ...
  const handleMeetCreated = useCallback(
    (apiResult) => {
  ...
  const handleDeleteForMe = (id) => {
    if (Array.isArray(id)) {
      id.forEach((i) => {
        deleteMessageForMe(chatId, [i]);
        setLocalDeletes((prev) => new Set(prev).add(i));
      });
    } else {
  ...
  const handleDeleteForAll = (id) => {
    if (Array.isArray(id)) {
      id.forEach((i) => {
        deleteMessageForAll(chatId, [i]);
        setLocalDeletesForAll((prev) => new Set(prev).add(i));
      });
    } else {
  ...
  if (!chatId)
    return (
      <div className="chatEmpty">
        ...
        <div className="muted chatEmptySubtitle">
  ...
  return (
    <div className="chat">
      ...
      <style>{`
        ...
        .pollOptionVoted { cursor: pointer; }
        .pollOptionContent {
          display: flex;
          align-items: center;
        ...
        .pollProgressBar {
          ...
          height: 4px; background: #e9edef; border-radius: 0 0 8px 8px;
        }
        .pollProgressFill {
          height: 100%; background: #d1d5db;
          border-radius: 0 0 8px 8px; transition: width 0.4s ease;
        }
        ...
        .pollModalHeader {
          padding: 20px 24px;
          background: linear-gradient(135deg, #00a884 0%, #00876a 100%);
          color: #fff;
        }
        .pollModalTitle {
        ...
        .pollInputLabel {
          font-size: 13px; font-weight: 600; color: #00a884;
          text-transform: uppercase; letter-spacing: 0.5px;
          margin-bottom: 8px; display: block;
        }
        .pollQuestionInput {
          width: 100%; padding: 14px 16px;
          border: 2px solid #e9edef; border-radius: 12px;
        ...
        .pollOptionNumber {
          ...
          display: flex; align-items: center; justify-content: center;
          font-size: 13px; font-weight: 600; color: #667781; flex-shrink: 0;
        }
        .pollOptionInput {
          flex: 1; padding: 12px 14px;
          border: 2px solid #e9edef; border-radius: 10px;
        ...
        .pollAddOptionBtn {
          ...
          background: transparent; color: #00a884; font-size: 14px;
        ...
        .pollModalFooter {
          ...
          padding: 16px 24px; background: #f9fafb; border-top: 1px solid #e9edef;
        }
        .pollCancelBtn {
          padding: 12px 24px; border: 1px solid #d1d5db; border-radius: 10px;
        ...
        .infoCardSection { background: #fff; margin-bottom: 10px; padding: 14px 20px; border-radius: 8px; margin: 10px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); }
          ...
          <div className="selectionHeaderContent">
            ...
            <div className="selectionActions">
              <button
                ...
                onClick={() => {
                  selectedMessages.forEach((id) => {
                    const current =
                      localStars[id] !== undefined
                        ? localStars[id]
                        : combinedMessages.find((m) => m.id === id)?.isStarred;
              ...
              <button
                ...
                title="Delete"
                onClick={() =>
                  setDeletePrompt({ id: selectedMessages, type: "me" })
                }
              >
                🗑️
              </button>
            ...
            >
              ...
              <div className="chatHeaderText">
                <div className="chatHeaderName">
                  {chatInfo?.name}{" "}
                  {isAdminBlocked ? "🚫" : isPersonalBlock ? "🔒" : ""}
                </div>
      ...
      {pinnedMessages.length > 0 && !searchOpen && !selectionMode && (
        <div
          className="pinnedMessagesBanner"
          onClick={() => {
            const node = messageRefs.current[pinnedMessages[0].id];
        ...
        >
          ...
          <div>
            ...
            <div className="pinnedSnippet">
              {pinnedMessages[0].text || "Attachment"}
            </div>
          </div>
        </div>
      )}
                ...
                {m.type === "poll" || m.messageType === "poll" ? (
                  ...
                  <MessageBubble
                    ...
                    onReaction={(emoji) => {
                      ...
                      sendReaction(m.id, r[emoji] || "ok");
                    }}
                    onStar={() => {
                      const current =
                        localStars[m.id] !== undefined
                          ? localStars[m.id]
                          : m.isStarred;
                      setLocalStars((prev) => ({
                        ...prev,
                    ...
                    onDeleteForMe={() =>
                      setDeletePrompt({ id: m.id, type: "me" })
                    }
                    onDeleteForAll={() =>
                      setDeletePrompt({ id: m.id, type: "all" })
                    }
      ...
      {amISuspended ? (
        <footer
          className="composer composerDisabled"
        ...
        <footer className="composer composerDisabled">
        ...
        <footer className="composer">
          ...
          {editingMessage && (
            <div className="waComposerReplyBar animatedFadeIn">
              <div
                className="waComposerReplyAccent"
          ...
          <FileUploadButton
            onFileSelect={(f) => {
              if (canPost) sendFile(f);
            }}
            disabled={!isConnected || !canPost}
          />
          <MeetButton
            targetId={!isGroup ? targetId : null}
          ...
          <button
            className="sendBtn"
            onClick={handleSend}
            disabled={!text.trim() || !isConnected || !canPost}
          >
              ...
              <div className="pollInputGroup">
                ...
                <div className="pollOptionsContainer">
                  {pollOptions.map((option, index) => (
                    <div key={index} className="pollOptionInputRow">
                      <div className="pollOptionNumber">{index + 1}</div>
                      <input
                        type="text"
                  ...
                  >
                    ...
                    {pollOptions.length < 12 &&
                      `(${12 - pollOptions.length} remaining)`}
                  </button>
                </div>
              </div>
              ...
              <div className="pollSettingsSection">
                <div className="pollSettingRow">
                  <div className="pollSettingInfo">
                    <div className="pollSettingTitle">
                      Allow multiple answers
                    </div>
                    <div className="pollSettingDesc">
          ...
          >
            ...
            <div className="drawerBody">
              <div className="infoProfileHero">
                ...
                <h1 className="infoNameDisplay">
                  ...
                  {isGroup && isGroupAdmin && (
                    <span className="editPencilSmall">✎</span>
                  )}
                </h1>
                <p className="infoStatusDisplay">
                  {isGroup
                    ? `Group · ${chatInfo?.memberCount} members`
              ...
              <div className="infoCardSection">
                <label
                  className={
                    isGroup ? "sectionLabelGreen" : "sectionLabel"
                  }
                >
                ...
                <div className="infoValueText">
                  {isGroup
                    ? chatInfo?.description ||
                      "Official team discussion group."
                    : chatInfo?.about || "Available for chat"}
                </div>
              </div>
              ...
              {isGroup && (
                <div className="infoCardSection">
                  ...
                  >
                    <label className="sectionLabelGreen">
                      Saved Bookmarks
                    </label>
                    <span
                      style={{
                  ...
                  <div
                    className="infoValueText"
                    style={{ fontSize: "13px", color: "#667781" }}
                  >
                  ...
                  >
                    Group created by {chatInfo?.createdBy || "Admin"} on{" "}
                    {new Date(
                      chatInfo?.createdAt || Date.now()
                    ).toLocaleDateString()}
                  </div>
                </div>
              )}
              ...
              >
                <div>
                  ...
                  {isGroup && (
                    ...
                    >
                      No media, docs or links shared yet.
                    </div>
                  )}
                </div>
                <span
              ...
              {isGroup && (
                ...
                >
                  ...
                  <input
                    type="checkbox"
                    checked={chatInfo?.isBroadcast || false}
                    readOnly
                    style={{
                      width: "18px",
                      height: "18px",
              ...
              >
                ...
                >
                  Make messages in this chat disappear after the selected
                  time. Pinned messages will be kept.
                </div>
              </div>
                    ...
                    <div style={{ marginBottom: "15px" }}>
                      ...
                      <div style={{ display: "flex", gap: "10px" }}>
                        <select
                          ...
                          onChange={(e) =>
                            setAddMemberSelect(e.target.value)
                          }
                        >
                          ...
                          {chats
                            .filter(
                              (c) =>
                                c.kind === "dm" &&
                                !chatInfo?.members?.some(
                                  (m) =>
                                    String(m.id) === String(c.odooId)
                                )
                            )
                            .map((u) => (
                              <option key={u.odooId} value={u.odooId}>
                                {u.name}
                              </option>
                            ))}
                        </select>
              ...
              >
                {!isGroup && (
                  <div className="footerBtnGrid">
                    {isAdmin ? (
                      <button
                        className="footerActionBtn gray"
                        onClick={() => {
                      ...
                      <button
                        ...
                        onClick={() => {
                          toggleBlockChat(chatId);
                          showToast(
                            isPersonalBlock ? "Unblocked" : "Blocked",
                            "success"
                          );
                        }}
                      >
                    ...
                    {isAdmin ? (
                      <button
                        className="footerActionBtn orange"
                        onClick={() => {
                          if (
                            window.confirm(
                              "WARNING: This completely deletes the employee. Continue?"
                            )
                          ) {
                            deleteEmployeeGlobally(chatId);
                            navigate("/chats");
                          }
                        }}
                      >
              ...
              {mediaMessages.length === 0 ? (
                ...
                mediaMessages.map((m) => (
                  <div
                    ...
                    onClick={() => setPreviewMedia(m.fileUrl)}
                  >
                    {m.type === "image" ? (
                      <img
                        src={m.fileUrl}
      ...
      {showForwardModal && (
        ...
        >
          ...
          >
            ...
            >
              <h3 style={{ margin: 0, color: "#111b21", fontSize: "18px" }}>
                Forward to...
              </h3>
            </div>
            <div
            ...
            >
              {chats.map((c) => (
                <label
                  ...
                  onMouseOver={(e) =>
                    (e.currentTarget.style.background = "#f5f6f6")
                  }
                  onMouseOut={(e) =>
                ...
                >
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked)
                        setForwardTargets((prev) => [...prev, c.id]);
                      else
                  ...
                  />
                  <div style={{ fontWeight: 500, color: "#111b21" }}>
                    {c.kind === "group" ? "👥 " : ""}
                    {c.name}
                  </div>
                </label>
            ...
            >
              ...
              <button
                style={{
                  ...
                  borderRadius: "8px",
                  fontWeight: 600,
                  opacity: forwardTargets.length === 0 ? 0.5 : 1,
                  cursor:
                    forwardTargets.length === 0 ? "not-allowed" : "pointer",
                }}
                disabled={forwardTargets.length === 0}
                onClick={() => {
                  forwardMessages(
                    Array.isArray(forwardingMessage)
                      ? forwardingMessage
                      : [forwardingMessage],
                    forwardTargets
                  );
        ...
        >
          ...
          >
            ...
            <p className="customModalText">
              ...
              {Array.isArray(deletePrompt.id)
                ? "these messages"
                : "this message"}{" "}
              {deletePrompt.type === "all" ? "for everyone" : "for yourself"}?
            </p>
            <div className="customModalActions">
              ...
              {deletePrompt.type === "all" ? (
                <button
                  ...
                  onClick={() => handleDeleteForAll(deletePrompt.id)}
                >
                  Delete for Everyone
                </button>
              ) : (
                <button
                  ...
                  onClick={() => handleDeleteForMe(deletePrompt.id)}
                >
                  Delete for Me
                </button>
              )}
      ...
      {isGroup && activeThreadMsgId && activeThreadMsg && (
        ...
        >
          <aside
            className="chatInfoDrawer threadDrawer"
            onClick={(e) => e.stopPropagation()}
          >
            <div
              className="chatInfoDrawerHeader"
              style={{
                padding: "15px 20px",
                background: "#fff",
            ...
            >
              ...
              >
                ...
                >
                  {activeThreadMsg.senderName}
                </div>
                <div
                  className="threadMsgText"
                  style={{
                    fontSize: "15px",
                    color: "#111b21",
                    marginBottom: "8px",
                ...
                >
                  {formatTime(activeThreadMsg.createdAt)}
                </div>
              </div>
              <div
                className="threadRepliesList"
                style={{
                  display: "flex",
              ...
              >
                {activeThreadMsg.thread?.map((tm) => (
                  <div
                    key={tm.id}
                    className="threadReplyItem"
                    style={{
                      padding: "10px 15px",
                      borderRadius: "10px",
                      alignSelf:
                        tm.sender === "me" ? "flex-end" : "flex-start",
                      background:
                        tm.sender === "me" ? "#d9fdd3" : "#fff",
                      maxWidth: "85%",
                    }}
                  >
                    {tm.sender !== "me" && (
